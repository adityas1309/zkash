...
async function run() {
  ...
  // In circomlibjs, poseidon() returns Uint8Array.
  // We need to know if that Uint8Array representation matches what poseidon EXPECTS as input when passed as bigint.

  // 'F.toObject(element)' returns bigint.
  // 'F.e(x)' converts to element.

  const precommitmentElement = poseidon([nullifier, secret]);
  const precommitmentAsBigIntFromF = F.toObject(precommitmentElement);

  console.log('Precommitment (BE interpretation):', precommitmentBigInt);
  console.log('Precommitment (F.toObject):       ', precommitmentAsBigIntFromF);

  if (precommitmentBigInt !== precommitmentAsBigIntFromF) {
    console.log(
      'MISMATCH! ensure `bytesToBigInt(poseidon_output)` matches `F.toObject(poseidon_output)`',
    );
  }
