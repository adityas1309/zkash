...
async function checkDeployFee() {
    try {
        ...
        const wasm = fs.readFileSync('packages/contracts/groth16_verifier/target/wasm32v1-none/release/groth16_verifier.wasm');

        // Get account sequence
        const account = await new SDK.Horizon.Server('https://horizon.stellar.org').loadAccount(publicKey);
