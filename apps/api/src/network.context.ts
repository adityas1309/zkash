...
export function getContractAddress(name: 'GROTH16_VERIFIER_ADDRESS' | 'SHIELDED_POOL_ADDRESS' | 'ZK_SWAP_ADDRESS' | 'SHIELDED_POOL_XLM_ADDRESS'): string {
    ...
    if (!value) {
        console.warn(`[getContractAddress] Missing environment variable for ${isMainnet ? 'Mainnet' : 'Testnet'} ${name}`);
    }
    return value || '';
}
...
export function getHorizonUrl(): string {
    const isMainnet = isMainnetContext();
    const defaultUrl = isMainnet ? 'https://horizon.stellar.org' : 'https://horizon-testnet.stellar.org';
    const prefix = isMainnet ? 'MAINNET_' : 'TESTNET_';
...
export function getSorobanRpcUrl(): string {
    const isMainnet = isMainnetContext();
    const defaultUrl = isMainnet ? 'https://mainnet.sorobanrpc.com' : 'https://soroban-testnet.stellar.org';
    const prefix = isMainnet ? 'MAINNET_' : 'TESTNET_';
