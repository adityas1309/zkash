...
class WitnessCalculator {
    ...
    async _doCalculateWitness(input_orig, sanityCheck) {
        try {
            ...
            keys.forEach((k) => {
                ...
                for (let i = 0; i < fArr.length; i++) {
                    try {
                        ...
                        const msg = `[WitnessCalculator] ERROR processing signal '${k}' at index ${i}. Value: ${JSON.stringify(fArr[i])} (Type: ${typeof fArr[i]}) - ${e.message}`;
                        console.error(msg);
                        throw new Error(msg);
                    }
            ...
            console.error('[WitnessCalculator] FATAL ERROR in _doCalculateWitness:', e);
            throw e;
        }
    }
