      ...
      <div className="relative z-10 mx-auto flex w-full max-w-[1450px] flex-col">
        ...
        <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-bold text-white">Plan the ramp before you move money</h1>
            <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-400">
              This workspace compares buy and sell routes, highlights provider and inventory blockers, and turns fiat
              actions into stage-based execution plans.
            </p>
          </div>
          <button
        ...
        <div className="mb-8 grid w-full gap-4 md:grid-cols-4">
          <Card variant="glass">
            ...
            <p className="mt-2 text-lg font-bold text-white">{workspace?.provider.razorpayConfigured ? "Configured" : "Missing"}</p>
          ...
          <Card variant="glass">
            ...
            <p className="mt-2 text-lg font-bold text-white">{workspace?.pricing.buyRateInrToXlm ?? 0}</p>
          ...
          <Card variant="glass">
            ...
            <p className="mt-2 text-[11px] text-slate-400">Public {workspace?.balances.publicXlm ?? 0} | Private {workspace?.balances.privateXlm ?? 0}</p>
          </Card>
          <Card variant="glass">
            <p className="text-xs uppercase tracking-[0.18em] text-slate-500">Payout Hold</p>
            <p className="mt-2 text-lg font-bold text-white">{workspace?.payoutPolicy.holdMinutes ?? 0}m</p>
            <p className="mt-2 text-[11px] text-slate-400">{workspace?.readiness.payoutRail.detail}</p>
          </Card>
        </div>
        ...
        <div className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
          <div className="rounded-[32px] border border-white/5 bg-slate-900/80 p-6 backdrop-blur-2xl">
              ...
              <div className="flex gap-2 rounded-xl border border-slate-700/50 bg-slate-800/30 p-1">
                <button
                  ...
                  className={cn(
                    ...
                    activeTab === "buy" ? "bg-slate-700 text-white" : "text-slate-400 hover:text-white",
                  )}
                >
                  Buy XLM
                </button>
                <button
                  ...
                  className={cn(
                    "rounded-lg px-4 py-2 text-xs font-semibold transition-all",
                    activeTab === "sell" ? "bg-slate-700 text-white" : "text-slate-400 hover:text-white",
                  )}
                >
                  Sell XLM
                </button>
            ...
            <div className="space-y-5">
              ...
              {activeTab === "buy" ? (
                <div>
                  <label className="mb-1.5 ml-1 block text-xs font-medium text-slate-400">Receive Mode</label>
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    >
                      <Wallet className={cn("mx-auto mb-2", mode === "public" ? "text-indigo-400" : "text-slate-500")} size={18} />
                      <span className="block text-xs font-semibold text-slate-200">Public</span>
                      <span className="mt-0.5 block text-[10px] text-slate-500">Visible wallet</span>
                    </button>
                    <button
                      onClick={() => setMode("zk")}
                      className={cn(
                    ...
                    >
                      <Shield className={cn("mx-auto mb-2", mode === "zk" ? "text-indigo-400" : "text-slate-500")} size={18} />
                ...
                <>
                  <div>
                    <label className="mb-1.5 ml-1 block text-xs font-medium text-slate-400">Account Number</label>
                    <Input
                      type="text"
                      value={accountNo}
                  ...
                  <div>
                    <label className="mb-1.5 ml-1 block text-xs font-medium text-slate-400">IFSC Code</label>
                    <Input
                      type="text"
                      value={ifsc}
          ...
          <div className="space-y-6">
            <Card variant="neon">
              <div className="flex items-center justify-between gap-4">
                <div>
                  <p className="text-xs uppercase tracking-[0.18em] text-slate-500">Execution plan</p>
                  <h2 className="mt-2 text-xl font-semibold text-white">
                    {plan ? plan.readiness.headline : "Preview a buy or sell route"}
                  </h2>
                  <p className="mt-2 text-sm leading-6 text-slate-400">
                    {plan ? plan.readiness.detail : "Once you enter an amount, the desk will turn the ramp into explicit stages and route cards."}
                  </p>
                </div>
                {planLoading ? (
              ...
              <Card variant="glass">
                ...
                <div className="space-y-3">
                  {(plan?.routeCards ?? workspace?.scenarioCards ?? []).map((card: any) => (
                    <div key={card.route ?? card.id} className="rounded-2xl border border-slate-800 bg-slate-950/70 p-4">
                      <div className="flex items-center justify-between gap-3">
                        <p className="text-sm font-medium text-white">{card.title}</p>
                        {"recommended" in card && card.recommended && <Badge variant="success">Recommended</Badge>}
                      </div>
              ...
              <Card variant="glass">
                ...
                <div className="space-y-3">
                  ...
                  {(plan?.warnings ?? []).map((warning) => (
                    <div key={warning} className="rounded-2xl border border-yellow-500/20 bg-yellow-500/10 p-4 text-sm text-yellow-100">
                      {warning}
                    </div>
                  ))}
                </div>
                  ...
                  {plan?.bankAccount ? (
                    <>
                      ...
                      <div className="rounded-2xl border border-slate-800 bg-slate-950/70 p-4">
                        ...
                        <p className="mt-2 text-sm text-white">{plan.bankAccount.validation.detail}</p>
                      </div>
                    </>
                  ) : (
              ...
              <Card variant="glass">
                ...
                <div className="space-y-3">
                  {plan?.inventory ? (
                    <>
                      <div className="rounded-2xl border border-slate-800 bg-slate-950/70 p-4">
                        <p className="text-xs uppercase tracking-wide text-slate-500">Public inventory</p>
                        <p className="mt-2 text-sm text-white">{plan.inventory.publicXlm} XLM</p>
                      </div>
                      <div className="rounded-2xl border border-slate-800 bg-slate-950/70 p-4">
                        <p className="text-xs uppercase tracking-wide text-slate-500">Private inventory</p>
                      ...
                      <div className="rounded-2xl border border-slate-800 bg-slate-950/70 p-4">
                        ...
                        <p className="mt-2 text-sm text-white">{plan.inventory.inventoryShortfall} XLM</p>
                      </div>
                    </>
                  ) : (
                    <div className="rounded-2xl border border-slate-800 bg-slate-950/70 p-4 text-sm text-slate-300">
                      Total XLM inventory: {workspace?.balances.totalXlm ?? 0}, split across public and private lanes.
                    </div>
                  )}
                </div>
              </Card>
        ...
        <div className="mt-8 flex items-center justify-center opacity-70">
          <p className="flex items-center gap-2 text-xs text-slate-400">
            <Sparkles size={12} /> Planning the fiat ramp before execution reduces avoidable checkout, inventory, and payout failures.
          </p>
        </div>
      </div>
    </div>
